...
def elegir_funcion():
    ...
    print("Funciones para Simpson 3/8: ")
...
def simpson38(f = None):
    ...
    n = int(input("Ingrese el número de subintervalos (debe ser múltiplo de 3): "))
    
    # Validar que n sea múltiplo de 3
    if n % 3 != 0:
        print("El número de subintervalos debe ser múltiplo de 3.")
        return
    ...
    encabezado = ["N", "Integral Aproximada", "Integral Exacta", "Error Absoluto", "Error Porcentual"]
...
def grafica(f=None):
    ...
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.title('Gráfico de la función con S3/8')
    plt.legend()
